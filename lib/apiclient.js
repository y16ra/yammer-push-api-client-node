...
var util = require('util'),
    ...
    PromiseHttpClient = require('promise-http-client'),
    logger = require("log4js").getLogger("net.renalias.yammer.pushapi");
...
function YammerPushAPI(oauthToken, config, httpClient) {
    ...
    this.client = httpClient || new PromiseHttpClient();
}
...
YammerPushAPI.prototype.realtimeResponseHandler = function(handshake) {
    ...
    then(function(response) {
        ...
        setTimeout(function() {
            try {
                self.realtimeResponseHandler(handshake); 
            } catch(e) {
        ...
        setTimeout(function() {
            try {
                ...
                console.log("Error realtimeResponseHandler: " + e);
            }
        }, 10000);
    });
};
...
YammerPushAPI.prototype.doHandshake = function() {
    ...
    logger.info("Starting handshake process; token = " + this.oauthToken);

    var req = self.initiateRealtimeHandshake.call(self)
    .then(function(body) {
...
/**
 * This method initiates the real-time handshake by calling the desired endpoint (all, topic or group)
 * and providing the authorization credentials in the right header.
 *
 * Returns a promise object.
 */
YammerPushAPI.prototype.initiateRealtimeHandshake = function() {
    var headers = {
        "Content-Type": "application/json", // FIXME: I should not need to specify this if I'm calling getAsJson!
        "Authorization": "Bearer " + this.oauthToken
    }
    return(this.client.getAsJson(this.yammerBase + this.endpointUrl, headers));
}
